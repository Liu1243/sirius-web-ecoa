...
export const ImportLibraryDialog = ({ open, title, onClose }: ImportLibraryDialogProps) => {
  const { t } = useTranslation('sirius-web-application', { keyPrefix: 'importLibraryDialog' });
    ...
    <Dialog open={open} onClose={onClose} fullWidth maxWidth="lg" scroll="paper" data-testid="import-library">
      ...
      <DialogActions>
        {state.actions.length > 0 ? (
          <Typography variant="caption">{t('applyToLibraries', { count: state.selectedLibraries.length })}</Typography>
        ) : (
    ...
    <div ref={widgetRef}>
      <ButtonGroup
        ...
        <Button
          ...
          loading={loading}
          onClick={handleClick}>
          {actions.length > state.selectedIndex ? t(actions[state.selectedIndex].id, { defaultValue: actions[state.selectedIndex].label }) : t('noAction')}
        </Button>
        <Button
          color="primary"
          size="small"
