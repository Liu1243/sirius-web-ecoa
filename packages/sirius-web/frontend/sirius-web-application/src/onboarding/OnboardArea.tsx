...
const getExistingRepresentationsForOnboardQuery = gql`
  query getExistingRepresentationsForOnboard($editingContextId: ID!, $targetObjectId: ID!) {
    viewer {
      editingContext(editingContextId: $editingContextId) {
...
export const OnboardArea = ({ editingContextId, readOnly }: MainAreaComponentProps) => {
  ...
  const { data: existingRepresentationsData } = useQuery(getExistingRepresentationsForOnboardQuery, {
    variables: { editingContextId, targetObjectId: objectId },
    skip: !objectId,
  });
