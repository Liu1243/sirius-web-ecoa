...
export const WorkbenchOmnibox = ({ editingContextId, workbenchHandle, children }: WorkbenchOmniboxProps) => {
  const { t } = useTranslation('sirius-web-application', { keyPrefix: 'workbenchOmnibox' });
  ...
  useEffect(() => {
    if (!commandsLoading && commandsData) {
      const commands: OmniboxCommand[] = commandsData.viewer.workbenchOmniboxCommands.edges
      ...
      if (command.id === 'search') {
        ...
        const previousSearchCommands: OmniboxCommand[] = getPreviousSearches().map((value) => {
          return {
            ...
            icon: <SearchIcon />,
            description: t('searchDescription', { value }),
          };
        });
