...
export const ProjectBrowserOmnibox = ({ children }: ProjectBrowserOmniboxProps) => {
  const { t } = useTranslation('sirius-web-application', { keyPrefix: 'projectBrowserOmnibox' });
  ...
  useEffect(() => {
    if (!commandsLoading && commandsData) {
      const commands: OmniboxCommand[] = commandsData.viewer.projectsOmniboxCommands.edges
        .filter((edge) => edge.node.id !== 'search')
        .map((edge) => ({
          id: edge.node.id,
  ...
  const handleCommandClick = (mode: OmniboxMode, command: OmniboxCommand) => {
    if (mode === 'Command') {
      ...
      if (splitCommandId.length == 2) {
        ...
        addErrorMessage(t('unsupportedSearchResult', { id: command.id }));
      }
    }
  };
