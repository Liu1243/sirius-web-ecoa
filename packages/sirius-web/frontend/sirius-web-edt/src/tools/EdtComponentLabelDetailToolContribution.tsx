...
type Modal = 'dialog';
export const EdtComponentLabelDetailToolContribution = ({ diagramElementId }: DiagramPaletteToolComponentProps) => {
  const [modal, setModal] = useState<Modal | null>(null);
  const { classes } = useToolStyle();
  const { t } = useTranslation('sirius-web-application', { keyPrefix: 'edtTools' });
  const nodes = useNodes<Node<NodeData>>();
  const targetedNode = nodes.find((node) => node.id === diagramElementId);
  if (
    !targetedNode ||
    !targetedNode.data.targetObjectKind.startsWith('siriusComponents://semantic?domain=edtproject&entity=Component')
  ) {
    return null;
  }
