...
public class ProjectUploadApplicationService implements IProjectUploadApplicationService {
    ...
    public IPayload uploadProject(UploadProjectInput input) {
        ...
        if (optionalProjectZipContent.isPresent()) {
            ...
            // renameAs lets the caller override the project name embedded in the ZIP (e.g. to resolve a name conflict)
            String projectName = (input.renameAs() != null && !input.renameAs().isBlank())
                    ? input.renameAs().trim()
                    : projectZipContent.projectName();

            var natures = this.getNatures(projectZipContent.manifest().get(ProjectZipContent.NATURES));
            IResult<Project> result = this.projectCreationService.createProject(new InitializeProjectInput(input.id(), input, projectZipContent), projectName, natures);
            ...
            } else if (result instanceof Failure<Project> failure) {
