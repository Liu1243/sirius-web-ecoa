...
public class ProjectCurrentEditingContextDataFetcher implements IDataFetcherWithFieldCoordinates<DataFetcherResult<String>> {
    ...
    private static final Logger LOGGER = LoggerFactory.getLogger(ProjectCurrentEditingContextDataFetcher.class);
    ...
    public DataFetcherResult<String> get(DataFetchingEnvironment environment) throws Exception {
        ...
        var optionalEditingContextId = this.projectEditingContextApplicationService.getEditingContextId(project.id(), name);
        LOGGER.info("{} Project currentEditingContext lookup projectId={} name={} resolvedEditingContextId={}", LOG_PREFIX, project.id(), name, optionalEditingContextId.orElse(null));
