...
public class SemanticDataProjectContentImportParticipant implements IProjectContentImportParticipant {
    ...
    private void copyDocuments(ICause cause, String editingContextId, Map<String, UploadFile> uploadFiles) {
        ...
        this.logger.info("[COPY-DOCS] findById took {}ms, found={}",
                System.currentTimeMillis() - t0, optionalIEMFEditingContext.isPresent());
        ...
        if (optionalIEMFEditingContext.isPresent()) {
            ...
            this.logger.info("[COPY-DOCS] Loaded {} documents in {}ms, semanticIds={}, docIds={}",
                    uploadFiles.size(), System.currentTimeMillis() - tLoad,
                    semanticIds.size(), documentIds.size());
            ...
            // NOTE: the actual DB write (transaction commit + SemanticDataUpdatedEvent) fires AFTER this
            // method returns; the gap between this log and [REP-IMPORT] is the DB commit time.
            this.logger.info("[COPY-DOCS] persist() returned in {}ms (DB commit + rep-import happen next)",
                    System.currentTimeMillis() - tPersist);

            this.logger.info("[COPY-DOCS] Total copyDocuments: {}ms", System.currentTimeMillis() - t0);
        }
    }
