...
public class MutationUploadProjectDataFetcher implements IDataFetcherWithFieldCoordinates<IPayload> {
    ...
    private static final String RENAME_AS = "renameAs";
    ...
    public IPayload get(DataFetchingEnvironment environment) throws Exception {
        ...
        if (optionalId.isPresent() && optionalFile.isPresent()) {
            ...
            var renameAs = Optional.ofNullable(input)
                    .map(map -> map.get(RENAME_AS))
                    .map(Object::toString)
                    .orElse(null);

            if (hasCapability) {
                payload = this.projectUploadApplicationService.uploadProject(new UploadProjectInput(inputId, uploadFile, renameAs));
            } else {
