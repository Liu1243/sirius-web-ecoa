...
public class MutationUploadImageDataFetcher implements IDataFetcherWithFieldCoordinates<IPayload> {
    ...
    public MutationUploadImageDataFetcher(IProjectImageApplicationService imageApplicationService, ICapabilityEvaluator capabilityEvaluator, IMessageService messageService) {
    ...
    public IPayload get(DataFetchingEnvironment environment) throws Exception {
        ...
        if (optionalId.isPresent() && optionalProjectId.isPresent() && optionalLabel.isPresent() && optionalFile.isPresent()) {
            ...
            var hasCapability = this.capabilityEvaluator.hasCapability(SiriusWebCapabilities.PROJECT, projectId, SiriusWebCapabilities.Project.EDIT);
            if (!hasCapability) {
                return new ErrorPayload(id, this.messageService.unauthorized());
            }
