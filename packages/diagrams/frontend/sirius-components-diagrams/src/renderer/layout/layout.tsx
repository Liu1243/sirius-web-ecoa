  ...
  visibleNodes.forEach((node, index) => {
    if (hiddenContainer && node) {
      ...
      if (isRectangularNode(node)) {
        // Strip connectionHandles to avoid React Flow Handle "No node id found" warning
        // when rendering nodes outside of a React Flow NodeIdContext (layout measurement only)
        const layoutData: FreeFormNodeData = { ...node.data, connectionHandles: [] };
      ...
      if (isListNode(node)) {
        // Strip connectionHandles to avoid React Flow Handle "No node id found" warning
        // when rendering nodes outside of a React Flow NodeIdContext (layout measurement only)
        const layoutData: ListNodeData = { ...node.data, connectionHandles: [] };
