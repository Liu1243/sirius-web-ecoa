...
import { defaultNodeMargin } from './layoutParams';
import {
...
export class ListNodeLayoutHandler implements INodeLayoutHandler<ListNodeData> {
  ...
  private handleParentNode(
    ...
    const siblingGap = Math.max(defaultNodeMargin, 0);
    const totalSiblingGap = Math.max(directNodesChildren.length - 1, 0) * siblingGap;
    ...
    previousChildrenContentBoxHeightToConsider -= node.data.topGap + node.data.bottomGap + totalSiblingGap;
    ...
    directNodesChildren.forEach((child, index) => {
      ...
      if (previousSibling) {
        child.position = {
          ...child.position,
          y: previousSibling.position.y + (previousSibling.height ?? 0) + siblingGap,
        };
      }
    });
    ...
    const minComputeChildrenHeight =
      directNodesChildren
        .map((node) => (node.data.resizedByUser ? node.height ?? 0 : node.data.minComputedHeight ?? 0))
        .reduce((sum, height) => sum + height, 0) + totalSiblingGap;
