...
export const getChildNodePosition = (
  ...
  const startX = rectangularNodePadding + borderWidth + maxWestBorderNodeWidth;
  const startY = borderWidth + headerHeightFootprint + maxNorthBorderNodeHeight;
  ...
  if (!previousSibling) {
    ...
    // If the parent container has a known width and the child would overflow, wrap to next row
    const parentAvailableWidth = parentNode?.width
      ? parentNode.width - borderWidth * 2 - rectangularNodePadding - maxWestBorderNodeWidth
      : undefined;

    if (parentAvailableWidth !== undefined && candidateX + (child.width ?? 0) > parentAvailableWidth + startX) {
      return {
        x: startX,
        y: previousSibling.position.y + (previousSibling.height ?? 0) + gap,
      };
    }
