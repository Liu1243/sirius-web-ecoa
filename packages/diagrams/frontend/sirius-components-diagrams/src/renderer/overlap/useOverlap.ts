      ...
      if (overlapNode) {
        if (overlapNode.data.pinned) {
          if (direction === 'horizontal') {
            movingNode.position = {
              ...movingNode.position,
              x: overlapNode.position.x + (overlapNode.width ?? 0) + overlapGap,
            };
          } else {
            movingNode.position = {
              ...movingNode.position,
              y: overlapNode.position.y + (overlapNode.height ?? 0) + overlapGap,
            };
          }
          ...
          if (direction === 'horizontal') {
            overlapNode.position = {
              ...overlapNode.position,
              x: movingNode.position.x + (movingNode.width ?? 0) + overlapGap,
            };
          } else {
            overlapNode.position = {
              ...overlapNode.position,
              y: movingNode.position.y + (movingNode.height ?? 0) + overlapGap,
            };
          }
