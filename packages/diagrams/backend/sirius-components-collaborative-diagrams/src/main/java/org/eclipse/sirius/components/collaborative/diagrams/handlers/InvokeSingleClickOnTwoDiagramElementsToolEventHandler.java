...
public class InvokeSingleClickOnTwoDiagramElementsToolEventHandler implements IDiagramEventHandler {
    ...
    private static final Logger LOG = LoggerFactory.getLogger(InvokeSingleClickOnTwoDiagramElementsToolEventHandler.class);
        ...
        if (diagramInput instanceof InvokeSingleClickOnTwoDiagramElementsToolInput input) {
            ...
            if (diagramDescription.isPresent() && sourceDiagramElement.isPresent() && targetDiagramElement.isPresent()) {
                ...
                if (status instanceof Success success && success.getParameters().get(Success.NEW_SELECTION) instanceof WorkbenchSelection newSelection) {
                    ...
                    LOG.info(
                            "invokeSingleClickOnTwoDiagramElementsTool failed: toolId={} sourceId={} targetId={} messages={}",
                            input.toolId(),
                            input.diagramSourceElementId(),
                            input.diagramTargetElementId(),
                            failure.getMessages()
                    );
                    payload = new ErrorPayload(diagramInput.id(), failure.getMessages());
                } else if (status instanceof Success success) {
                    // The mutation will look "successful" at the GraphQL transport layer (200, no GraphQL errors),
                    // but nothing will change if the handler returns Success without the NEW_SELECTION parameter.
                    LOG.info(
                            "invokeSingleClickOnTwoDiagramElementsTool returned Success without NEW_SELECTION: toolId={} sourceId={} targetId={} messages={} parametersKeys={}",
                            input.toolId(),
                            input.diagramSourceElementId(),
                            input.diagramTargetElementId(),
                            success.getMessages(),
                            success.getParameters() == null ? null : success.getParameters().keySet()
                    );
                }
            } else {
                LOG.info(
                        "invokeSingleClickOnTwoDiagramElementsTool could not run (missing context): toolId={} sourcePresent={} targetPresent={} diagramDescriptionPresent={}",
                        input.toolId(),
                        sourceDiagramElement.isPresent(),
                        targetDiagramElement.isPresent(),
                        diagramDescription.isPresent()
                );
            }
