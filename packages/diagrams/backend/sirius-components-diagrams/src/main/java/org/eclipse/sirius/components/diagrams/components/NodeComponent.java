...
public class NodeComponent implements IComponent {
    ...
    private static final Logger LOGGER = LoggerFactory.getLogger(NodeComponent.class);
    ...
    public Element render() {
        ...
        if (this.props.getContainmentKind() == NodeContainmentKind.BORDER_NODE) {
            LOGGER.info("NodeComponent candidates descriptionId={} containmentKind={} parentElementId={} semanticElementsCount={}",
                    nodeDescription.getId(), this.props.getContainmentKind(), this.props.getParentElementId(), semanticElements.size());
            this.logEndpointParentState(nodeDescription.getId(), nodeComponentVariableManager);
        }
        ...
        for (Object semanticElement : semanticElements) {
            ...
            if (this.shouldRender(targetObjectId, optionalPreviousNode, nodeVariableManager)) {
                ...
                if (this.props.getContainmentKind() == NodeContainmentKind.BORDER_NODE) {
                    LOGGER.info("NodeComponent rendered descriptionId={} targetObjectId={} containmentKind={} parentElementId={}",
                            nodeDescription.getId(), targetObjectId, this.props.getContainmentKind(), this.props.getParentElementId());
                }
            ...
            } else if (this.props.getContainmentKind() == NodeContainmentKind.BORDER_NODE) {
                LOGGER.info("NodeComponent skipped descriptionId={} targetObjectId={} containmentKind={} parentElementId={}",
                        nodeDescription.getId(), targetObjectId, this.props.getContainmentKind(), this.props.getParentElementId());
            }
    ...
    private void logEndpointParentState(String descriptionId, VariableManager variableManager) {
        Object self = variableManager.getVariables().get(VariableManager.SELF);
        if (self == null) {
            return;
        }
        String eClassName = self.getClass().getSimpleName();
        if (!(eClassName.contains("ServiceOfLinkedComponentDefinition")
                || eClassName.contains("ModuleInstance")
                || eClassName.contains("ReferenceOfLinkedComponentDefinition")
                || eClassName.contains("ComponentImplementation"))) {
            return;
        }

        Object operationsValue = this.invokeGetter(self, "getOperations");
        Object serviceDefinitionLink = this.invokeGetter(self, "getServiceDefinitionLink");
        Object componentServiceLink = this.invokeGetter(self, "getComponentDefinitionServiceLink");
        Object componentReferenceLink = this.invokeGetter(self, "getComponentDefinitionReferenceLink");
        int operationsCount = this.sizeOf(operationsValue);
        int serviceDefinitionOperationsCount = this.serviceDefinitionOperationsCount(serviceDefinitionLink);
        int componentServiceSyntaxOperationsCount = this.contractSyntaxOperationsCount(componentServiceLink);
        int componentReferenceSyntaxOperationsCount = this.contractSyntaxOperationsCount(componentReferenceLink);

        LOGGER.info(
                "NodeComponent endpointParent descriptionId={} parentEClass={} operationsCount={} serviceDefinitionOperationsCount={} componentServiceSyntaxOperationsCount={} componentReferenceSyntaxOperationsCount={} serviceDefinitionLinkClass={} componentServiceLinkClass={} componentReferenceLinkClass={}",
                descriptionId,
                eClassName,
                operationsCount,
                serviceDefinitionOperationsCount,
                componentServiceSyntaxOperationsCount,
                componentReferenceSyntaxOperationsCount,
                this.classNameOf(serviceDefinitionLink),
                this.classNameOf(componentServiceLink),
                this.classNameOf(componentReferenceLink));
    }
    ...
    private int serviceDefinitionOperationsCount(Object serviceDefinitionLink) {
        if (serviceDefinitionLink == null) {
            return -1;
        }
        return this.sizeOf(this.invokeGetter(serviceDefinitionLink, "getOperations"));
    }

    private int contractSyntaxOperationsCount(Object contract) {
        if (contract == null) {
            return -1;
        }
        Object syntax = this.invokeGetter(contract, "getSyntax");
        if (syntax == null) {
            return -1;
        }
        return this.sizeOf(this.invokeGetter(syntax, "getOperations"));
    }
