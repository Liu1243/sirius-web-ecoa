...
public class NodeChildrenComponent implements IComponent {
    ...
    private static final Logger LOGGER = LoggerFactory.getLogger(NodeChildrenComponent.class);
    ...
    private List<Element> getBorderNodes(Optional<Node> optionalPreviousNode, String nodeId) {
        ...
        LOGGER.info("NodeChildren borderDescriptions parentDescriptionId={} nodeId={} borderDescriptionIds={}",
                nodeDescription.getId(), nodeId,
                borderNodeDescriptions.stream().map(NodeDescription::getId).toList());
