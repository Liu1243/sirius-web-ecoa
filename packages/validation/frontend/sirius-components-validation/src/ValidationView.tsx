    ...
    if (state.validationPayload && !complete) {
      ...
      const accordions = processedValidation.categories.map((category) => {
        const details = category.diagnostics.reduce<React.ReactNode[]>((acc, diagnostic, index) => {
          ...
          let messageToRender = diagnostic.message;
          messageToRender = messageToRender.replace(
            /The required feature '(.+?)' of '(.+?)' must be set/g,
            (match, feature, object) => {
              return t('requiredFeature', { feature, object, defaultValue: match }) as string;
            }
          );
          messageToRender = messageToRender.replace(
            /The feature '(.+?)' of '(.+?)' with (\d+) values must have at least (\d+) values/g,
            (match, feature, object, current, min) => {
              return t('featureMinValues', { feature, object, current, min, defaultValue: match }) as string;
            }
          );

          acc.push(<Typography key={diagnostic.id}>{messageToRender}</Typography>);
          return acc;
        }, []);

        const countText = t('diagnosticCount', { count: category.diagnostics.length });

        return (
          <Accordion key={category.kind}>
            <AccordionSummary expandIcon={<ExpandMoreIcon />}>
              <Typography className={classes.heading}>{category.kind}</Typography>
              <Typography className={classes.secondaryHeading}>{countText}</Typography>
            </AccordionSummary>
