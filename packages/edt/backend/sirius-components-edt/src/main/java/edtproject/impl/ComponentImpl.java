		...
		if (componentReferences == null) {
			...
			public void addUnique(ComponentReference object) {
				for (int i = 0; i < componentReferences.size(); i++) {
					ComponentReference componentReference = componentReferences.get(i);
					if (!Objects.equals(componentReference, object)
						...
						// 用持久化对象（有 UUID）替换临时对象（由 createRefServPropFromDef 创建，无 UUID）
						// 这样 ServiceLink.source 引用的 UUID 在重启后仍然有效
						super.remove(componentReference);
						break;
					}
				}
		...
		if (componentServices == null) {
			componentServices = new EObjectContainmentEList<ComponentService>(ComponentService.class, this,
				...
				public void addUnique(ComponentService object) {
					for (int i = 0; i < componentServices.size(); i++) {
						ComponentService componentService = componentServices.get(i);
						if (!Objects.equals(componentService, object)
							...
							// 用持久化对象（有 UUID）替换临时对象（由 createRefServPropFromDef 创建，无 UUID）
							// 这样 ServiceLink.target 引用的 UUID 在重启后仍然有效
							super.remove(componentService);
							break;
						}
					}
