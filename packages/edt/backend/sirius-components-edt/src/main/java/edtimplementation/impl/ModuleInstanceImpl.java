...
public class ModuleInstanceImpl extends MinimalEObjectImpl.Container implements ModuleInstance {
	...
	private void createPropertyValueFromModuleType() {
		if (moduleType != null) {
			...
			// Remove only orphaned PropertyValues (propertyType no longer in ModuleType).
			// Existing PropertyValues with a valid propertyType are preserved together with
			// any user-set value — clearing them on every setModuleType call would silently
			// discard values that were persisted to the database.
			if (propertyValues != null) {
				Iterator<PropertyValue> iter = propertyValues.iterator();
				while (iter.hasNext()) {
					PropertyValue pv = iter.next();
					if (pv.getPropertyType() == null || !edtModuleTypeProperty.contains(pv.getPropertyType())) {
						iter.remove();
					}
				}
			}
			// Add PropertyValues only for ModuleType properties not yet represented.
			for (ModuleTypeProperty propertyType : edtModuleTypeProperty) {
				boolean alreadyPresent = getPropertyValues().stream()
						.anyMatch(pv -> propertyType.equals(pv.getPropertyType()));
				if (!alreadyPresent) {
	...
	private void createOpFromModuleType() {
		if (moduleType != null) {
			...
			if (operations != null) {
				// If any existing operation still has an unresolved proxy MTOperationRef, we
				// are in the middle of EMF JSON deserialization (cross-references not yet
				// resolved).  In that case, skip both the cleanup step and the creation step:
				// the existing operations loaded from JSON are valid and will have their
				// MTOperationRef proxies resolved once all objects are loaded.  Creating new
				// OperationInstances here would produce duplicates; cleaning would incorrectly
				// remove valid operations (proxy != resolved object → contains() returns false).
				boolean loadingFromJson = operations.stream().anyMatch(this::hasUnresolvedMTOperationRef);
				if (loadingFromJson) {
					return;
				}
	...
	private void cleanModuleInstanceOperations(EList<ModuleOperation> edtModuleTypeOperations) {
		...
		while (each.hasNext()) {
			...
			// Skip operations whose MTOperationRef is still an unresolved proxy.
			// During EMF JSON deserialization, setModuleType() may be called before
			// the MTOperationRef cross-references have been resolved.  In that case
			// the proxy will not be found in edtModuleTypeOperations (the EList.contains()
			// comparison against a proxy object fails), so moduleOperationAlreadyInherited()
			// would wrongly return true and delete valid operations — together with the
			// associated DataLink/EventLink/RequestLink objects.  Skipping proxy-MTOperationRef
			// operations here preserves both the operations and the links until the model is
			// fully loaded and resolution can be done correctly.
			if (hasUnresolvedMTOperationRef(op)) {
				continue;
			}
	...
	/**
	 * Returns {@code true} if the OperationInstance has a non-null MTOperationRef
	 * that is still an unresolved EMF proxy (i.e., the cross-reference has not yet
	 * been resolved during loading).  Proxy references must not be used for equality
	 * checks against resolved objects because the comparison will always fail.
	 */
	private boolean hasUnresolvedMTOperationRef(OperationInstance op) {
		Object ref = null;
		if (op instanceof DataReaderInstance dr) {
			ref = dr.getMTOperationRef();
		} else if (op instanceof DataWriterInstance dw) {
			ref = dw.getMTOperationRef();
		} else if (op instanceof EventReceiverInstance er) {
			ref = er.getMTOperationRef();
		} else if (op instanceof EventSenderInstance es) {
			ref = es.getMTOperationRef();
		} else if (op instanceof RequestClientInstance rc) {
			ref = rc.getMTOperationRef();
		} else if (op instanceof RequestServerInstance rs) {
			ref = rs.getMTOperationRef();
		}
		return ref instanceof InternalEObject internalEObject && internalEObject.eIsProxy();
	}
	...
	private boolean alreadyExistsInInstanceOperation(OperationInstance object, OperationInstance operation) {
		return (object instanceof DataReaderInstance newDr && operation instanceof DataReaderInstance opDr
				&& (newDr.getMTOperationRef() != null && opDr.getMTOperationRef() != null
				&& Objects.equals(opDr.getMTOperationRef(), newDr.getMTOperationRef())))
				|| (object instanceof DataWriterInstance newDw && operation instanceof DataWriterInstance opDw
				&& (newDw.getMTOperationRef() != null && opDw.getMTOperationRef() != null
				&& Objects.equals(opDw.getMTOperationRef(), newDw.getMTOperationRef())))
				|| (object instanceof EventSenderInstance newEs && operation instanceof EventSenderInstance opEs
				&& (newEs.getMTOperationRef() != null && opEs.getMTOperationRef() != null
				&& Objects.equals(opEs.getMTOperationRef(), newEs.getMTOperationRef())))
				|| (object instanceof EventReceiverInstance newEr && operation instanceof EventReceiverInstance opEr
				&& (newEr.getMTOperationRef() != null && opEr.getMTOperationRef() != null
				&& Objects.equals(opEr.getMTOperationRef(), newEr.getMTOperationRef())))
				|| (object instanceof RequestClientInstance newRc && operation instanceof RequestClientInstance opRc
				&& (newRc.getMTOperationRef() != null && opRc.getMTOperationRef() != null
				&& Objects.equals(opRc.getMTOperationRef(), newRc.getMTOperationRef())))
				|| (object instanceof RequestServerInstance newRs && operation instanceof RequestServerInstance opRs
				&& (newRs.getMTOperationRef() != null && opRs.getMTOperationRef() != null
				&& Objects.equals(opRs.getMTOperationRef(), newRs.getMTOperationRef())));
	}
	...
	private boolean alreadyExistsInInstancePinfo(PinfoValue object, PinfoValue pinfoValue) {
		return ((object instanceof PublicPinfoValue newObject && pinfoValue instanceof PublicPinfoValue publicPinfoValue
				...
				|| (object instanceof PrivatePinfoValue newPrivateObject
				&& pinfoValue instanceof PrivatePinfoValue privatePinfoValue
				&& (newPrivateObject.getPrivatePinfoFromModuleType() != null
				&& privatePinfoValue.getPrivatePinfoFromModuleType() != null
				&& Objects.equals(privatePinfoValue.getPrivatePinfoFromModuleType(),
				newPrivateObject.getPrivatePinfoFromModuleType()))));
	}
