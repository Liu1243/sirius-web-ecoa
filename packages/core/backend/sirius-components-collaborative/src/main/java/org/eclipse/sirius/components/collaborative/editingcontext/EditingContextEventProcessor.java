...
public class EditingContextEventProcessor implements IEditingContextEventProcessor {
    ...
    public Mono<IPayload> handle(IInput input) {
        ...
        try {
            ...
            // Ensure payloadSink is always set: if dispatch threw an exception the sink would never
            // be filled, causing any downstream blockOptional() call to hang indefinitely.
            String errorMessage = exception.getMessage() != null ? exception.getMessage() : "Unexpected error during input dispatch";
            payloadSink.tryEmitValue(new ErrorPayload(input.id(), errorMessage));
        }
