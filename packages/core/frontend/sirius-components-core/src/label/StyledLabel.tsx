...
const useTreeItemStyle = makeStyles()((theme: Theme) => ({
  selected: {
...
export const getTextFromStyledString = (styledString: GQLStyledString): string => {
...
const styledLabelStyleStruckOut = (gqlLabelStyle: GQLStyledStringFragmentStyle): React.CSSProperties => {
  let textDecorations: String[] = [];
  if (gqlLabelStyle.isStruckOut) {
  ...
  return styledString.styledStringFragments.map((fragment, index) => {
    return (
      <span key={index} style={styledLabelStyle(fragment.styledStringFragmentStyle)}>
        <span style={styledLabelStyleStruckOut(fragment.styledStringFragmentStyle)}>
          {fragment.text}
        </span>
      </span>
    );
