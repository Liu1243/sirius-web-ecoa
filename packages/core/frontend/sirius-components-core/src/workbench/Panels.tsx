...
export const Panels = forwardRef<WorkbenchPanelsHandle | null, PanelsProps>(
  (
    ...
    useImperativeHandle(
      ...
      [leftContributions, leftPanelState, rightContributions, rightPanelState]
    );
    ...
    const toggleLeftPanel = useCallback((isOpen: boolean) => {
      setLeftPanelState((prevState) => ({ ...prevState, isOpen }));
    }, []);

    const toggleRightPanel = useCallback((isOpen: boolean) => {
            ...
            {leftPanelState.isOpen ? (
              <PanelGroup direction="vertical">
                {leftSelectedContributions.map((leftContribution, index) => (
                  <React.Fragment key={`left-fragment-${leftContribution.id}`}>
                    <Panel id={leftContribution.id} order={index} className={classes.panel} minSize={10}>
                      <WorkbenchPart
            ...
            {rightPanelState.isOpen ? (
              <PanelGroup direction="vertical">
                {rightSelectedContributions.map((rightContribution, index) => (
                  <React.Fragment key={`right-fragment-${rightContribution.id}`}>
                    <Panel id={rightContribution.id} order={index} className={classes.panel} minSize={10}>
                      <WorkbenchPart
                        editingContextId={editingContextId}
                        readOnly={readOnly}
                        side="right"
