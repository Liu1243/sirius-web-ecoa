...
import { VirtualMenuAnchor } from './context-menu/TreeItemContextMenu.types';
...
export const TreeItem = ({
  ...
  const onContextMenu: React.MouseEventHandler<HTMLDivElement> = (event) => {
    ...
    const anchor: VirtualMenuAnchor = {
      nodeType: 1,
      getBoundingClientRect: () => new DOMRect(clientX, clientY, 0, 0),
    };
    setState((prevState) => ({ ...prevState, rightClickMenuAnchor: anchor }));
  };

  const closeRightClickMenu = () => {
    setState((prevState) => ({ ...prevState, rightClickMenuAnchor: null }));
  };
  ...
  if (textToFilter && isFilterCandidate(item, textToFilter)) {
    ...
    const rightClickContextMenu = state.rightClickMenuAnchor ? (
      <TreeItemContextMenu
        menuAnchor={state.rightClickMenuAnchor}
        editingContextId={editingContextId}
        treeId={treeId}
        item={item}
        readOnly={readOnly}
