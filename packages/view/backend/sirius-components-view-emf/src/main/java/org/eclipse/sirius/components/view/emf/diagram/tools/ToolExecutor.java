...
public class ToolExecutor implements IToolExecutor {
    ...
    public IStatus executeTool(Tool tool, AQLInterpreter interpreter, VariableManager variableManager) {
        ...
        if (result.status() == OperationExecutionStatus.FAILURE) {
            // Keep the returned Failure message stable for end-users, but log detailed context for troubleshooting.
            if (LOG.isWarnEnabled()) {
                try {
                    LOG.warn(
                            "Tool execution failed: toolName='{}' feedbackMessages={} variableKeys={}",
                            tool.getName(),
                            feedbackStackedMessages,
                            variableManager == null ? null : variableManager.getVariables().keySet()
                    );
                } catch (RuntimeException logFailure) {
                    // Never let logging issues hide the original failure.
                    LOG.warn("Tool execution failed and context logging also failed: toolName='{}'", tool.getName(), logFailure);
                }
            }
