...
static ldp_status_t ldp_create_server_udp_socket(ldp_r_socket_ctx* socket_ctx, apr_pool_t *mp, ldp_socket_info* tcp_info){
...
static ldp_status_t ldp_create_client_udp_socket(ldp_w_socket_ctx* socket_ctx, apr_pool_t *mp, ldp_socket_info* tcp_info){
	apr_status_t ret;
#if USE_AF_UNIX
    char* l_addr = apr_psprintf(mp, "/tmp/%s_%d", tcp_info->addr, tcp_info->port);
	ret=apr_sockaddr_info_get(&socket_ctx->socket_addr, l_addr, APR_UNIX, tcp_info->port, 0, mp);
