...
void ldp_start_comp_server(ldp_PDomain_ctx* ctx){
	...
	for (int i=0; i<ctx->nb_client; i++){
		if (interface_ctx_array[i].type == LDP_ELI_MCAST){
			...
			interface_ctx_array[i].inter.local.info_w = malloc(sizeof(ldp_socket_info));
			memcpy(interface_ctx_array[i].inter.local.info_w, interface_ctx_array[i].inter.local.info_r, sizeof(ldp_socket_info));
	...
	for (int i=0; i<ctx->nb_server; i++){
		if (interface_ctx_array[ctx->nb_client+i].type == LDP_ELI_MCAST){
			...
			interface_ctx_array[ctx->nb_client+i].inter.local.info_r = malloc(sizeof(ldp_socket_info));
			memcpy(interface_ctx_array[ctx->nb_client+i].inter.local.info_r, interface_ctx_array[ctx->nb_client+i].inter.local.info_w, sizeof(ldp_socket_info));
