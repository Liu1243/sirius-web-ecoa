...
class ECOA_Generate_Config(ECOA_Global_Config):
    ...
    def generate(self, file_log_dir, force_flag, debug_flag, coverage_flag,
        ...
        # In multi-platform mode, each `platform` binary only manages PDs deployed on
        # its own computing platform. Passing all PDs would make PD_number > 1, causing
        # the platform to wait for remote PDs that connect to *their* platform instead.
        local_pds = {name: pd for name, pd in self.deployment.protection_domains.items()
                     if pd.platform == current_PF.name}
