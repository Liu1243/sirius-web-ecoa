...
ECOA__error_type_INITIALISATION_PROBLEM, 11);\n\
    ...
    for pf_link_id, pf_link in PF_links.items():
        ...
        if current_platform.name == pf_link.source_platform:
            ...
            # PF_link does not involve the current platform, skip it
            continue
    ...
    # In multi-platform simulation both `platform` and PD binaries run in the SAME
    # container and share the same IP.  If `platform` also binds the ELI receive
    # socket (e.g. UDP:30001), it conflicts with PD_Reader_PD's own ELI receive
    # socket on the same port.  The platform binary only needs to SEND ELI platform-
    # status messages; all incoming ELI data (platform-status AND VD service ops) is
    # received directly by the PD binary whose router handles service ops and ignores
    # platform-status messages (PD_proc_ELI_management_msg → do-nothing).
    text += "    ctx.mcast_reader_interface_num = 0; \n"
