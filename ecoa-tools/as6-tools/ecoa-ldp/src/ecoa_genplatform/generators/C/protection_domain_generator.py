...
def protection_domain_ctx_gen(directory, pd, components, component_implementations, wires, libraries,
    ...
    # ELI read Sockets
    # PD binaries own the ELI receive socket directly (peer-to-peer between PDs).
    # The `platform` binary no longer binds the ELI receive port (mcast_reader_interface_num=0),
    # so there is no bind conflict even when platform and PD run in the same container.
    ...
    for binding_file, PF_link_ID_list in protection_domain.external_PF_reading_socket_index.items():
        ...
        if tmp_str_binding != "":
            ...
            for index, pf_link_id in enumerate(PF_link_ID_list):
                # sender_PF_ID in the ELI UDP header is set to UDP_current_PF_ID of the
                # SENDING platform's interface, which equals {link}_sent_PF_ID from the
                # SENDER'S route.h (= its own ELI platform ID).
                # From the RECEIVER's route.h, that same ID is {link}_read_PF_ID
                # (= find_mcast_PF_id(other_PF) = sender's ELI platform ID).
                # We must use _read_PF_ID here because this code runs on the RECEIVER.
