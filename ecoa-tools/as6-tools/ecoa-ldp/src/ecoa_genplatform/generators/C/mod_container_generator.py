...
def generate_H_container(output_directory, mimpl, mtype, libraries,
    ...
    # Include container_types.h (generated by MSCIGT) before user_context.h so that
    # the data-handle typedefs are visible when user_context.h is processed.
    # This also avoids re-defining the same typedefs inline below (which would
    # conflict with the definitions already in container_types.h).
    print("#include \"" + mname + "_container_types.h\"", file=fd)
    ...
    for (_, op) in mtype.get_operations().items():
        if op.get_type() == 'ES':
            ...
            # Data-handle typedef is already defined in container_types.h (MSCIGT-generated)
            # and included above. Only emit the function declarations here.
            print(
                "ECOA__return_status " + mname + "_container__" + op.get_name()
