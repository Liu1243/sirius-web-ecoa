...
def generate_main_cmake(directory, components, protections_domains, debug_flag, coverage_flag, force_flag):
    ...
    text += "set(CMAKE_USE_DDS_PROTO false CACHE BOOL \"DDS protocol\")\n"
    text += "set(CMAKE_USE_UDP_PROTO false CACHE BOOL \"UDP protocol\")\n"
    text += "# DDS Domain ID (0-232): injected from dds-binding.xml via -DCYCLONEDDS_DOMAIN_ID=N\n"
    text += "set(CYCLONEDDS_DOMAIN_ID 0 CACHE STRING \"CycloneDDS Domain ID (0-232)\")\n"
    text += "if(CMAKE_USE_DDS_PROTO)\n"
    text += "    set(CMAKE_USE_UDP_PROTO false CACHE BOOL \"UDP protocol\" FORCE)\n"
    text += "    message(\"-- Use DDS protocol (domain=${CYCLONEDDS_DOMAIN_ID})\")\n"
    text += "    add_definitions(-D USE_DDS_PROTO=1 -D USE_UDP_PROTO=0)\n"
    text += "    add_definitions(-D LDP_DDS_DOMAIN_ID=${CYCLONEDDS_DOMAIN_ID})\n"
    text += "elseif(CMAKE_USE_UDP_PROTO)\n"
    text += "    message(\"-- Use UDP protocol\")\n"
    text += "    add_definitions(-D USE_DDS_PROTO=0 -D USE_UDP_PROTO=1)\n"
    text += "else()\n"
    text += "    message(\"-- Use TCP protocol\")\n"
    text += "    add_definitions(-D USE_DDS_PROTO=0 -D USE_UDP_PROTO=0)\n"
    text += "endif ()\n\n"
